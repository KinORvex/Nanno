...
def create_new_observation(
    ...
    if observation_in.environmental_reading_id is not None:
        reading = db.get(EnvironmentalReading, observation_in.environmental_reading_id)
        if reading is None or reading.project.owner_id != current_user.id:
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Leitura ambiental não encontrada.")
        if reading.project_id != culture.experiment.project_id:
            raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, detail="A leitura ambiental não pertence ao projeto da cultura.")
